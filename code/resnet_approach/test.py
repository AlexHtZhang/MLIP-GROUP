...
data_path = path + '/human-protein/train/' # this path is temporary

df = pd.read_csv(path + '/data/test_idx.txt', sep = '\t', header = None)
df.columns = ['order']
order = list(df['order'])

# labels of one hot
label = np.load(path + '/data/test_one_hot.npy')

# id of pictures
testset = pd.read_csv(path + '/data/train.csv')
ls = testset['Id']
num = testset['Target']
...
for k in range(NB):
    i = k*B
    xtest = prepare.load_batch(ls,order,data_path,B,k)
...
res= [i/float(label.shape[0]) for i in res]
average_accuracy = sum(res)/28
print(res)#accuracy of every class
print(average_accuracy) #averaged accuracy
